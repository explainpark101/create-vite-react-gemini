/**
 * create-vite-react-gemini - Node.js 구현
 *
 * Node.js로 실행 시:
...
async function main() {
  ...
  try {
    execSync(`npx create-vite@latest ${targetDir} --template react`, {
      cwd,
      stdio: "pipe",
    });
  } catch (err) {
  ...
  execSync("npm install lucide-react tailwindcss @tailwindcss/vite", {
    cwd: projectDir,
    stdio: "inherit",
  });
...
${indexCss}`;
  ...
  writeFileSync(indexCssPath, newIndexCss);

  // 5. github actions 설정 추가
  const githubActionsPath = path.join(
    projectDir,
    ".github",
    "workflows",
    "deploy.yml"
    ...
    steps:
      ...
      - name: Deploy to GitHub Pages
        ...
        with:
          ...
          publish_dir: dist
  `.trim();
  writeFileSync(githubActionsPath, newGithubActions);
